...
namespace {
  ...
  struct DynamicWeightChoiceGenInfo {
    ...
    bool empty() const { return scaleWeightIDs.empty() && pdfWeightIDs.empty(); }
  };
...
class LHEWeightTableProducer : public edm::global::EDProducer<edm::StreamCache<LumiCacheInfoHolder>,
                                                               edm::RunCache<DynamicWeightChoice>,
                                                               edm::RunSummaryCache<CounterMap>,
                                                               edm::EndRunProducer> {
public:
  LHEWeightTableProducer(edm::ParameterSet const& params)
  ...
  void produce(edm::StreamID id, edm::Event& iEvent, const edm::EventSetup& iSetup) const override {
    ...
    if (lheInfo.isValid()) {
      ...
      fillLHEWeightTables(counter,
        ...
        edm::LogWarning("LHETablesProducer") << "Something is wrong here, no output will be produced\n";
    }
...
//        false);  // make sure only the first thread dumps out this (even if may still be mixed up with other output, but nevermind)
    ...
    counter->incLHE(w0, wScale, wPDF, wRwgt, wNamed);
  }
...
DEFINE_FWK_MODULE(LHEWeightTableProducer);
